...
import FinanceManagementTableRow from './FinanceManagementTableRow';

interface FinanceManagementTableProps {
  data: any[];
}

export default function FinanceManagementTable({ data }: FinanceManagementTableProps) {
  if (data.length === 0) return <p className="text-white">No data available.</p>;

  const headers = ['Transaction ID', 'Description', 'Amount', 'Date', 'Status', 'Created At', 'Action'];
    ...
    <div className="overflow-x-auto rounded-lg">
      <table className="w-full border-collapse text-gray-900">
       <thead className="bg-gray-100">
        ...
        <tbody>
          {data.map((row, index) => (
            <FinanceManagementTableRow key={index} row={row} />
          ))}
        </tbody>
      </table>
     {/* Pagination */}
