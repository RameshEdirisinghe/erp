...
import DataTableRow from './employeeDataTableRow';
...
export default function DataTable({ data }: DataTableProps) {
  if (!data || data.length === 0) {
    return <p className="text-gray-500 p-5">No data available.</p>;
  }

  const headers = ['ID', 'Name', 'Role', 'Salary', 'Action'];
  ...
  return (
    <div className="overflow-x-auto rounded-lg">
      <table className="w-full border-collapse text-gray-900">
        ...
        <tbody>
          {data.map((row) =>
            row ? <DataTableRow key={row.id} row={row} /> : null
          )}
        </tbody>
      </table>
